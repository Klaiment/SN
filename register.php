    ...
    <div class="w-full max-w-md space-y-8">
        ...
        <form class="mt-8 space-y-6" action="#" method="POST">
            <div class="-space-y-px rounded-md shadow-sm">
                <div>
                    <label for="pseudo" class="sr-only">Adresse email</label>
                    <input id="pseudo" name="pseudo" type="text" autocomplete="pseudo" required class="relative block w-full appearance-none rounded-none rounded-t-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="Pseudo">
                </div>
                <div>
          ...
          <span class="absolute inset-y-0 left-0 flex items-center pl-3">
            ...
            <div>
                <a href="/login.php"><button type="button" class="group relative flex w-full justify-center rounded-md border border-transparent bg-cyan-600 py-2 px-4 text-sm font-medium text-white hover:bg-cyan-700 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2">
          <span class="absolute inset-y-0 left-0 flex items-center pl-3">
...
if (isset($_POST['submiter'])){
    //check si on envoie le formulaire
        //verification que pseudo complet
        if (isset($_POST['pseudo']) && !empty($_POST['pseudo'])){
            ...
            if (isset($_POST['email']) && !empty($_POST['email'])){
                ...
                if (isset($_POST['password']) && !empty($_POST['password'])){
                    ...
                    $request = "INSERT INTO users (`pseudo`, `email`, `password`) VALUES (:pseudo,:email,:password)";
                    $req = $conn->prepare($request);
                    $req->bindParam(':pseudo',$_POST['pseudo']);
                    $req->bindParam(':email',$_POST['email']);
                    $req->bindParam(':password',$password);
                    $req->execute();
                    echo "<script>createsuccess('Compte crée')</script>";
                }else{
                    // Erreur Pseudo => Notif
            ...
            // Erreur Pseudo => Notif
            echo "<script>createerror('Merci de remplir votre pseudo !')</script>";
        }
